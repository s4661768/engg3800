...
void ILI9163_Init()
{
	...
	Displ_WriteCommand(ILI9163_PIXEL_FORMAT);
#ifdef RGB666
...
#endif
	Displ_WriteCommand(ILI9163_RGB_INTERFACE);
	Displ_WriteData((uint8_t *)"\x80",1);        // disable MISO pin

	Displ_WriteCommand(ILI9163_SLEEP_OUT);
	HAL_Delay(120);

	Displ_WriteCommand(ILI9163_DISPLAY_ON);
	HAL_Delay(5);
...
void Displ_SetAddressWindow(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2) {
	...
	Displ_WriteCommand(ILI9163_COLUMN_ADDR);
	Displ_WriteData(data, 4);

	((uint32_t *)data)[0]=(((y2 & 0xFF)<<24) | ((y2 & 0xFF00)<<8) | ((y1 & 0xFF)<<8) | ((y1 & 0xFF00)>>8) );
	Displ_WriteCommand(ILI9163_PAGE_ADDR);
	Displ_WriteData(data, 4);
	Displ_WriteCommand(ILI9163_MEMWR);
}
...
void Displ_Orientation(Displ_Orientat_e orientation){
	...
	switch(orientation) {
		case Displ_Orientat_0:
			data[0]=ILI9163_MADCTL_0DEG;
			_height = DISPL_HEIGHT;
			_width = DISPL_WIDTH;
			break;
		case Displ_Orientat_90:
			data[0]=ILI9163_MADCTL_90DEG;
			_height = DISPL_WIDTH;
			_width = DISPL_HEIGHT;
			break;
		case Displ_Orientat_180:
			data[0]=ILI9163_MADCTL_180DEG;
			_height = DISPL_HEIGHT;
			_width = DISPL_WIDTH;
			break;
		case Displ_Orientat_270:
			data[0]=ILI9163_MADCTL_270DEG;
			_height = DISPL_WIDTH;
			_width = DISPL_HEIGHT;
			break;
	}
	Displ_WriteCommand(ILI9163_MADCTL);
